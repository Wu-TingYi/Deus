...
void UBigProjectGameInstance::DestroyActorWhenContainName()
{
	...
	if (UGameplayStatics::DoesSaveGameExist("SaveFile-1", 0))
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, TEXT("DestroyedActors Exist"));

		UBigProjectSaveGame * SaveGameRef = Cast<UBigProjectSaveGame>(UGameplayStatics::LoadGameFromSlot("SaveFile-1", 0));
